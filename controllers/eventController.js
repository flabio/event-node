    ...
    try {
        ...
        const [draw, aproved_shepherd, aproved_higher, aproved, archived, canceled] = await Promise.all([
            EventStates('draw')
                ...
                .populate('organization_id', 'name')
                .populate('type_event_id','name'),
                EventStates('aproved_shepherd')
                .populate('user_id', ['first_name', 'last_name'])
                .populate('organization_id', 'name')
                .populate('type_event_id','name'),
                EventStates('aproved_higher')
                .populate('user_id', ['first_name', 'last_name'])
                .populate('organization_id', 'name')
                .populate('type_event_id','name'),
                EventStates('aproved')
...
const total=await Event.countDocuments({ active: true, state: 'draw' });
        res.json({
...
const EventStates=(state)=>{
    return  Event.find({ active: true, state: state });
}
